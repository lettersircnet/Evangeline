...
int chan::flushKickQueue()
{
	...
	if(toKick.entries() - sentKicks)
	{
		...
		if(max < 1) max = 1;
		if(max > 30) max = 30;
		if(opedBots.entries() > 1)
		{
			for(p = toKick.begin(); p && j < max; p++)
			{
		...
		if(!j && opedBots.entries() < 2)
			j = getRandomItems(MultHandle, toKick.begin(), toKick.entries() - sentKicks, max, KICK_SENT);
		return kick4(MultHandle, j) > 0;
	}
	return 0;
}
...
void chan::names(const char *owner)
{
	...
	net.sendOwner(owner, "-!- Evangeline: ", (const char *) name, "(", getModes(), "): Total of ", total, " nicks [",
...
void chan::cwho(const char *owner, const char *arg)
{
	...
	if(!i) net.sendOwner(owner, "[*] Evangeline: No matches found", NULL);
}
...
void chan::gotKick(const char *victim, const char *offender, const char *reason)
{
	...
	if((limit <= users.entries() - 1 || (flags & FLAG_I)) && kicked && (kicked->flags & HAS_I) &&
		kicker && myTurn(chset->INVITE_BOTS, kicker->hash32()))
		inv = 1;
...
chanuser *chan::gotJoin(const char *mask, int def_flags)
{
	...
	{
		char uh_buf[MAX_LEN];
		snprintf(uh_buf, sizeof(uh_buf), "%s@%s", (char *) p->ident, (char *) p->host);
		pendingLinkTry(p->nick, uh_buf);
	}
