...
void client::checkMyHost(const char *to,  bool justConnected)
{
	...
	else
	{
		if(userlist.wildFindHost(userlist.me(), mask) == -1)
		{
			...
			if(extendhost(mask, buf, MAX_LEN) && userlist.addHost(userlist.me(), buf, config.handle, NOW) != -1)
			{
				net.send(HAS_B, S_ADDHOST, " ", userlist.me()->name, " ", buf, NULL);
				net.sendOwner(to, "[*] ", (const char *) config.handle, ": IRC host \002", buf, "\002 added", NULL);
				++userlist.SN;
				ircConnFailDelay = 15;
...
void client::sendStatus(const char *name)
{
	...
	net.sendOwner(name, "Hi. I'm ", (const char *) config.handle, " and I'm running Evangeline ", S_DISPLAY_VERSION, NULL);
...
entServer *client::getRandomServer()
{
	//count servers
	int i, num;
	bool requireIpv6 = !config.myipv6.isDefault() || isValidIp(config.vhost) == 6;
...
#endif
		if(!config.server[i].isDefault() && (!requireIpv6 || config.server[i].getHost().isIpv6()))
			++num;
...
int client::connectToHUB()
{
	...
	DEBUG(printf("[*] Connecting to HUB: %s port %d\n",
		...
		if(config.currentHub->getHost().isIpv6())
			fd = doConnect6(config.currentHub->getHost().ip, config.currentHub->getPort(),
				config.myipv6.isDefault() ? (const char *) config.vhost : (const char *) config.myipv6, -1);
		else
#endif
	...
	if(fd > 0)
	{
		...
		{
			char hport[16];
			snprintf(hport, sizeof(hport), "%d", (int) config.currentHub->getPort());
			net.send(HAS_N, "[*] ", (const char *) config.handle, ": connecting to hub ", (const char *) config.currentHub->getHost().ip, " port ", hport, NULL);
		}

#ifdef HAVE_SSL
		if(config.currentHub->isSSL())
		{
			net.hub.enableSSL();
	...
	++config.currentHub->failures;
	{
		char hport[16];
		snprintf(hport, sizeof(hport), "%d", (int) config.currentHub->getPort());
		net.send(HAS_N, "[!] ", (const char *) config.handle, ": cannot connect to hub ", (const char *) config.currentHub->getHost().ip, " port ", hport, " (", strerror(errno), ")", NULL);
	}
	return -1;
}
...
#ifdef HAVE_IPV6
			if(s->getHost().isIpv6())
				n = doConnect6(s->getHost().ip, s->getPort(),
					config.myipv6.isDefault() ? (const char *) config.vhost : (const char *) config.myipv6, -1);
			else
#endif
...
{
	/* overloaded: offload pending enemy-kicks to another oped bot so an
	   ongoing takeover keeps momentum while this bot's penalty cools down
	   (handoff sends only to another bot: no IRC sends, no penalty cost) */
	if(penalty >= 10)
	{
		chan *hc = first;
		for(; hc; hc = hc->next)
			if(hc->toKick.entries() > hc->sentKicks)
				hc->handoffKicks();
		return;
	}
	chan *ch = first;
	...
	while(ch && !penalty)
	{
		...
		{
			srand();
			net.irc.send("TOPIC ", (const char *) ch->name, " :[\002Evangeline\002]: ", topics[rand() % count(topics)], NULL);
			userlist.chanlist[ch->channum].status &= ~SET_TOPIC;
			penalty++;
			return;
		}
