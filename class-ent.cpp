...
void entHost::reset()
{
	if((typesAllowed & ipv6) && !(typesAllowed & ipv4))
	{
		ip = "::";
		connectionString = "::";
	}
...
{
	return !strcmp(connectionString, "0.0.0.0") || !strcmp(connectionString, "::");
}

bool entHost::isIpv6()
{
	return (getConnectionStringType(connectionString) & ipv6) || isValidIp(ip) == 6;
}
...
options::event *entHost::setValue(const char *arg1, const char *arg2, const bool justTest)
{
	if(!strcmp(arg1, name))
	{
		...
		else if(type & domain)
		{
			...
			bool prefer6 = false;
	#ifdef HAVE_IPV6
			prefer6 = !config.myipv6.isDefault() || isValidIp(config.vhost) == 6;
			if(prefer6 && (typesAllowed & ipv6) && inet::gethostbyname(_arg, buf, AF_INET6))
				ok = true;
	#endif
			if(!ok && !prefer6 && (typesAllowed & ipv4) && inet::gethostbyname(_arg, buf, AF_INET))
				ok = true;
	#ifdef HAVE_IPV6
			if(!ok && !prefer6 && (typesAllowed & ipv6) && inet::gethostbyname(_arg, buf, AF_INET6))
				ok = true;
	#endif
