...
void chan::gotMode(const char *modes, const char *args, const char *mask)
{
	...
	for(i=0; i<MODES_PER_LINE; i++)
	{
		/* a MODE line that names a mode requiring an argument but fails to
		 * provide one (malformed / desynced input) must not be processed:
		 * downstream getUser(arg[i])/list lookups would dereference NULL. */
		if((mode[0][i] == '+' || mode[0][i] == '-')
			&& chanModeRequiresArgument(mode[0][i], mode[1][i])
			&& (!arg[i] || !*arg[i]))
			continue;

		/* op/voice modes must reference a user that is actually in the
		 * channel; otherwise getUser() returns NULL and every downstream
		 * argHandle->flags dereference would crash. */
		if((mode[0][i] == '+' || mode[0][i] == '-')
			&& (mode[1][i] == 'o' || mode[1][i] == 'v')
			&& !getUser(arg[i]))
			continue;
	...
	{
		tmp = ME.server.isupport.max_kick_targets;
		if(tmp < 1) tmp = 1;
		if(tmp > 30) tmp = 30;
		j = getRandomItems(multHandle, toKick.begin(), toKick.entries()-sentKicks, tmp, KICK_SENT);
		kick6(multHandle, j);
	}
