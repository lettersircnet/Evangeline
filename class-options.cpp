...
settings::settings()
{
	...
	registerObject(LINK_SEARCH_DELAY = entTime("link-search-delay", 1, 3600, 15));
	...
	registerObject(OPS_PER_MODE = entInt("ops-per-mode", 0, 3, 3));
	registerObject(ASK_FOR_OP_DELAY = entTime("ask-for-op-delay", 1, 60, 2));
	...
	/* how often (ms) the main event loop evaluates periodic actions */
	registerObject(EVENT_LOOP_TICK = entInt("event-loop-tick", 50, 2000, 250));
	/* adaptive anti-flood: penalty seconds to inject per flood-kill (scaled
	 * up on repeated kills) + window (s) within which repeated kills count */
	registerObject(FLOOD_BACKOFF = entInt("flood-backoff", 1, 300, 10));
	registerObject(FLOOD_KICK_WINDOW = entTime("flood-kick-window", 30, 6*3600, 600));
	/* when measured PING/PONG RTT (ms) exceeds this, hold senders back to
	   lag_ms/1000 seconds of penalty floor to ride out a congested link */
	registerObject(LAG_PENALTY_THRESHOLD = entInt("lag-penalty-threshold", 100, 10000, 2000));
}
...
chanset::chanset()
{
	...
	registerObject(PUNISH_BOTS = entPerc("punish-bots", -100, 0, -100));
...
CONFIG::CONFIG()
{
	...
	/* optional options :) */
	registerObject(realname = entString("realname", 1, 255, "Evangeline 1.1.5 (reborn)"));
	...
	registerObject(mainnick = entWord("main-nick", 1, 32, ""));
	...
	registerObject(botnetword = entWord("botnetword", 1, 255, "###\001The\001Evangeline\001Project\001###"));
