...
our ($antiptrace, $noircbacktrace, $ssl, $no_ssl, $little_endian, $big_endian, $cc_prefix, $cc_options, $ld_options, $disable_adns, $firedns);
...
sub dependencyHint
{
	return "\nInstall build dependencies with one of:\n" .
	       "  Debian/Ubuntu: sudo apt update && sudo apt install -y build-essential libssl-dev\n" .
	       "  Fedora:        sudo dnf install -y gcc gcc-c++ make openssl-devel\n" .
...
sub checkOpenSSL
{
	...
	{
		my ($inc, $libpath) = @$candidate;
		my $cmd = "${cc_prefix}g++ openssl.c $inc $libpath -lssl -lcrypto $lflags 2> /dev/null";
		if(!system($cmd))
		{
			my $desc = "$inc $libpath -lssl -lcrypto";
			$desc =~ s/^\s+|\s+$//g;
			print $desc ? "found ($desc)\n" : "found (-lssl -lcrypto)\n";
			$cflags .= "$inc " if $inc;
			$lflags .= "$libpath " if $libpath;
			$lflags .= "-lssl -lcrypto ";
			$cflags .= "-DHAVE_SSL ";
			$have{'openssl.c'} = 1;
			return 1;
...
sub getGccOptions
{
	...
	{
		die "Missing C compiler: ${cc_prefix}gcc\n" . dependencyHint();
	} 
    ...
    if(!$have{"${cc_prefix}g++"})
    {
        die "Missing C++ compiler: ${cc_prefix}g++\n" . dependencyHint();
    }
	...
	if(!$no_ssl)
	{
		if(!checkOpenSSL())
		{
			die "OpenSSL headers/libraries are required by default. Run ./configure --without-ssl only for a legacy non-SSL build.\n" . dependencyHint();
		}
	}
