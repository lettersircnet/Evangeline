...
/* deterministic per-bot credential shared by a slave bot and its hub without
   any password exchange: pass = MD5(key=MD5(botnetword), msg=handle). both
   sides compute the same 16 bytes, so the hub only needs to know the handle
   and the (plaintext) botnetword to admit a link. */
void MD5BotPass(unsigned char digest[16], const char *handle, const char *botnetword)
{
	unsigned char key[16];

	if(!handle || !*handle || !botnetword) return;

	MD5Hash(key, botnetword, strlen(botnetword));
	MD5Hash(digest, handle, strlen(handle), key, 16);
}
