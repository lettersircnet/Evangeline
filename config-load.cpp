...
void CONFIG::load(const char *file, bool decrypted)
{
	...
	if((creation && !decrypted) || autocrypt)
	{
		...
		if(autocrypt)
		{
			printf("[*] Continuing startup with crypted config '%s'\n", file);
			autocrypt = 0;
		}
		else
		{
...
#ifdef HAVE_SSL
	if(config.listenport || config.ssl_listenport)
	{
		...
		if(!SSL_CTX_use_certificate_file(inet::server_ctx, "server.cert", SSL_FILETYPE_PEM))
		{
	...
	if(config.listenport)
	{
		...
		{
			printf("[*] Opening IPv6 listening SSL socket at %s:%d\n", (const char *) config.myipv6, (int) config.listenport);
			if((net.listenfd = startListening6(config.myipv6, config.listenport)) < 1)
			{
	...
	if(config.ssl_listenport)
	{
			...
			if(!config.myipv6.isDefault())
			{
				printf("[*] Opening IPv6 listening SSL socket at %s:%d\n", (const char *) config.myipv6, (int) config.ssl_listenport);
				if((net.ssl_listenfd = startListening6(config.myipv6, config.ssl_listenport)) < 1)
				{
					printf("[-] Cannot open socket (%s)\n", strerror(errno));
					exit(1);
				}
