...
void createInitialConfig()
{
	...
	printMessage( "The options are explained at EVANGELINE.md" );
	...
	switch( bottype )
	{
		case 0:
			...
			readUserInput( "What port should the bot listen on for SSL encrypted connections from owners and slaves?", config.listenport );
		...
		case 1:
			...
			readUserInput( "What port should the bot listen on for SSL encrypted connections from leafs?", config.listenport );
	...
	if( readUserYesNo( "Do you want to set up the very basic and optional stuff?", false ) )
	{
		...
		//readUserInput( "Path where evangeline logfiles should be stored to", config.logfile );
		//readUserInput( "Loglevel for evangeline actions", config.loglevel );
		...
		readUserInput( "Which ctcp version do you want? 0 = none, 1 = evangeline, 2 = irssi, 3 = epic, 4 = lice, 5 = bitchx, 6 = dzony loker, 7 = luzik, 8 = mirc 6.14", config.ctcptype );
...
void bootstrapHubOwner(const char *file)
{
	pstring<> ownerpass, handle, pass;
	options::event *e;

	printMessage("Fast hub bootstrap mode");
	readUserInput("Owner password", ownerpass);
	readUserInput("Owner handle", handle);
	readUserInput("Owner user password", pass);

	if(ownerpass.len() < 8 || handle.len() < 1 || pass.len() < 8)
	{
		printError("ownerpass and user password must be at least 8 chars, handle must not be empty");
		exit(1);
	}
	...
	bool encrypted = false;
	{
		...
		if(f)
		{
			...
			encrypted = n >= sizeof(h.id) && (!memcmp(h.id, oldMagic, sizeof(h.id)) || !memcmp(h.id, newMagic, sizeof(h.id)));
		}
	}

	configOnly = 1;
	autocrypt = encrypted ? 0 : 1;
	config.load(file, !encrypted);
	configOnly = 0;
	...
	{
			printError("-n bootstrap is only valid for hub/main configs with listen and no hub entry");
			exit(1);
		}
		config.bottype = BOT_MAIN;

		config.ownerpass.setReadOnly(false);
	e = config.ownerpass.setValue("ownerpass", ownerpass, 0);
	if(!e || !e->ok)
	{
		printError((const char *) e->reason);
		exit(1);
	...
	if(!h)
		h = userlist.addHandle(handle, 0, X_FLAGS | HAS_P, 0, 0, config.handle);
	if(!h)
	{
		printError("cannot create owner handle");
		exit(1);
	}
	h->flags[GLOBAL] = X_FLAGS | HAS_P;
	char hbuf[MAX_LEN], pbuf[MAX_LEN];
	strncpy(hbuf, handle, MAX_LEN - 1);
	hbuf[MAX_LEN - 1] = '\0';
	strncpy(pbuf, pass, MAX_LEN - 1);
	pbuf[MAX_LEN - 1] = '\0';
	MD5Hash(h->pass, pbuf, strlen(pbuf));
	++userlist.SN;
		if(!userlist.save(config.userlist_file))
		{
			printError("cannot save userlist");
			exit(1);
		}
		userlist.reset();

		printMessage("Owner `%s' created and hub config is encrypted", (const char *) handle);
	config.ownerpass.setReadOnly(false);
	config.botnetword.setReadOnly(false);
	config.ctcptype.setReadOnly(false);
