...
int modeq::validate(modeq_ent *e, int &a, int &n)
{
	...
	else if(e->mode[0] == '-')
	{
		switch(e->mode[1])
		{
			...
			case 'k':
				if(a == 3) return -1;
				if(e->arg && ch->key && *ch->key && !strcmp(e->arg, ch->key))
				{
					++a;
					return 1;
				}
...
int modeq::flush(int prio)
{
	...
	modeq_ent *modeList[MODES_PER_LINE];
	...
	{
		/* PRIO_LOW must only piggyback when we are (near-)idle, but a strict
		 * zero budget lets sustained HIGH activity starve it indefinitely
		 * (e.g. keepout de-bans never being sent on a busy channel). Allow it
		 * to fire within a small budget; HIGH still gets priority because it
		 * is flushed before PRIO_LOW in the same pass. */
		if(penalty > 5)
			return 0;
	}
	else
	...
	if(num)
	{
		...
		/* Soften the penalty spike: a single batched MODE line used to cost
		 * 3*num+1 (up to 3*8+1 = 25 for a full line), which decays at 1/sec and
		 * stalled ALL channels for ~25s after one burst. Charging a flat per
		 * command plus per mode keeps flood pacing (one line = one command)
		 * while removing the pathological stall. num is bounded by a+n (<=8). */
		penalty += 3 + num;
		for(i=0; i<num; ++i)
		{
...
modeq::modeq_ent *modeq::find(const char *mode, const char *arg)
{
	...
	while(g)
	{
		...
		while(e)
		{
			if(!strncmp(e->mode, mode, 2) && e->arg && arg && !strcmp(e->arg, arg))
				return e;
