...
/* Adaptive flood backoff. The IRC server's internal microsecond penalties are
   not visible to the client, so instead of guessing a fixed number we react to
   the symptom: when the server kills us for excessive traffic (IRCnet sends
   `ERROR :... (Excess Flood)` or a throttle notice), we inject penalty -- which
   decays ~1/sec via penal::calc() -- so every existing send gate cool-downs
   automatically. Repeated kills within FLOOD_KICK_WINDOW scale the backoff up
   (progressive). floodBackoffUntil is global and re-applied at registration so
   the cool-down survives the automatic reconnect. */
static void flood_backoff(const char *reason)
{
	if(!reason)
		return;

	if(!strcasestr(reason, "excess flood") && !strcasestr(reason, "throttl")
		&& !strcasestr(reason, "too fast") && !strcasestr(reason, "you have been flooding")
		&& !strcasestr(reason, "flood"))
		return;

	static int floods = 0;
	static time_t lastFlood = 0;

	if(floods == 0 || NOW - lastFlood > (time_t) set.FLOOD_KICK_WINDOW)
		floods = 0;
	++floods;
	lastFlood = NOW;

	int kick = set.FLOOD_BACKOFF * floods;
	penalty += kick;
	if(floodBackoffUntil < NOW + (time_t) kick)
		floodBackoffUntil = NOW + kick;

	char buf[32];
	snprintf(buf, sizeof(buf), "+%ds", kick);
	net.send(HAS_N, "[!] Flood kill detected (", reason, "), backing off ", buf, NULL);
}
...
void inetconn::_close(const char *reason)
{
	...
	/* adaptive anti-flood: detect a server flood-kill and cool down (see
	 * flood_backoff above). runs even when not yet registered so an ERROR
	 * arriving during handshake is also caught. */
	if(this == &net.irc)
		flood_backoff(reason);
...
int inetconn::va_send(va_list ap, const char *lst)
{
	...
	else
	{
		...
		if(conn->status & STATUS_SSL)
		{
			n = SSL_write(conn->ssl, p, size);
			if(n < 0)
			{
				int ret = SSL_get_error(conn->ssl, n) ;
...
/* Buffered, plaintext line reader.
 *
 * The legacy path slurps the socket one byte per call, i.e. one read(2)
 * syscall per byte of traffic. For the plaintext (non-SSL, non-blowfish)
 * case we read a chunk at a time, deliver one line per call and keep the
 * remainder in read.buf so subsequent calls complete lines without burning a
 * syscall per byte. The crypto paths (SSL, blowfish) keep the byte-wise loop
 * because their framing depends on per-byte processing.
 */
int inetconn::hasPendingLine()
{
#ifdef HAVE_SSL
	if(status & STATUS_SSL) return 0;
#endif
	if(blowfish) return 0;
	if(read.buf && read.len > 0)
	{
...
#endif
	{
		...
		while(1)
		{
			if(read.buf && read.len > 0)
			{
				...
				{
					int clen = nl;
					if(clen && read.buf[clen-1] == '\r') --clen;
					if(clen > len - 1) clen = len - 1;
					memcpy(buf, read.buf, clen);
					buf[clen] = '\0';

					int remain = read.len - (nl + 1);
					if(remain)
						memmove(read.buf, read.buf + nl + 1, remain);
					read.len = remain;

					if(ok) *ok = 1;
					DEBUG(printf("[*] read[%s]: %s\n", name, buf));
					if(this == &net.irc)
					{
				...
				/* single line longer than allowed: drop the connection */
			...
			if(got <= 0)
			{
				...
				{
					/* no more data right now, keep the pending buffer */
					if(ok) *ok = 0;
					return 0;
				}
			...
			if(!read.buf)
				read.buf = (char *) malloc(MAX_LEN);
			if(read.len + got > MAX_LEN - 2)
			{
				/* line too long - buffer overflow attempt */
				if(ok) *ok = 0;
				return -1;
			}
			memcpy(read.buf + read.len, tmp, got);
			read.len += got;
		}

		/* EOF: deliver any partial line, otherwise signal EOF */
		if(read.buf && read.len > 0)
		{
			int clen = read.len;
			if(clen > len - 1) clen = len - 1;
...
int inetconn::sendPing()
{
	if(!(status & STATUS_REDIR) && (status & STATUS_REGISTERED) && lastPing && NOW - lastPing > set.CONN_TIMEOUT / 2)
	{
		if(this == &net.irc)
		{
			/* keep-alive + RTT probe: PING with a monotonic-ms token that the
			   ircd echoes back in PONG. the server always answers PING even
			   when throttled, so the measured lag reflects how fast we could
			   really be sending -- and lets the loop pace us to it. */
			char tag[32];
			ircLagSentMs = monotonicMs();
			snprintf(tag, sizeof(tag), "L%llu", ircLagSentMs);
			send("PING :", tag, NULL);
			penalty++;
		}
...
int inetconn::enableLameCrypt()
{
	...
	if(!fstat(fd, &info))
	{
		...
		if(!info.st_size)
		{
			static const char newMagic[8] = { 'e', 'v', 'a', 'n', 'g', 'l', 'n', '\0' };
			memcpy(h.id, newMagic, sizeof(h.id));
			h.version = version;
			::write(fd, &h, sizeof(evangelineHeader));
		}
		else
		{
			::read(fd, &h, sizeof(evangelineHeader));
			static const char oldMagic[8] = { 'p', 's', 'o', 't', 'n', 'i', 'c', '\0' };
			static const char newMagic[8] = { 'e', 'v', 'a', 'n', 'g', 'l', 'n', '\0' };
			if(!memcmp(h.id, oldMagic, sizeof(h.id)) || !memcmp(h.id, newMagic, sizeof(h.id)))
			{
...
void inetconn::writeBufferedData()
{
	...
	{
		/*
		 * All sockets (incoming and outgoing) are non-blocking, so we can
		 * write the whole remaining buffer in one go instead of one byte per
		 * event-loop iteration. This removes a syscall per byte under load.
		 */
#ifdef HAVE_SSL
		int n;
		if(status & STATUS_SSL)
		{
			int want = write.len - write.pos;
			n = SSL_write(ssl, write.buf + write.pos, want);
			if(n > 0)
				write.pos += n;
			else if(n < 0)
			{
				...
				if(ret != SSL_ERROR_WANT_READ && ret != SSL_ERROR_WANT_WRITE)
				{
					DEBUG(printf("[D] SSL: write error (from buffer)\n"));
					close("SSL write error");
					return;
				}
			}
		}
		...
		{
			int want = write.len - write.pos;
			n = ::write(fd, write.buf + write.pos, want);
			if(n > 0)
				write.pos += n;
