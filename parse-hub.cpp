...
void parse_hub(char *data)
{
			...
			*/
			...
			case 3:
			{
				if(!strcmp(arg[0], S_REGISTER))
				{
					...
					if(config.bottype == BOT_LEAF && strlen(arg[2]) && strlen(arg[3]))
					{
						options::event *e = config.alt[0].setValue("alt", arg[2], arg[3]);
						if(e && e->ok)
							config.alt[0].failures = 0;
					}
					...
					if(config.bottype != BOT_LEAF)
					{
						...
						net.hub.send(S_LINKINFO, " ", (const char *) config.handle, " ", host, " ", portbuf, NULL);
					}
	...
	/* S_BKICK <chan> <nick> ... - overloaded oped bot hands off its enemies */
	if(!strcmp(arg[0], S_BKICK) && strlen(arg[1]))
	{
		ch = ME.findChannel(arg[1]);
		if(ch && ch->me->flags & IS_OP)
		{
			for(int k = 2; k < 10 && strlen(arg[k]); ++k)
			{
				chanuser *u = ch->getUser(arg[k]);
				if(u && !(u->flags & HAS_F))
					ch->toKick.sortAdd(u);
			}
			if((int) penalty < 15)
	...
	if(config.bottype != BOT_MAIN && !strcmp(arg[0], S_OREDIR) && strlen(arg[4]))
	{
		char *a = srewind(data, 3);
		if(a) net.sendOwner(arg[2], "(", arg[1], ") ", a, NULL);
		return;
	}
