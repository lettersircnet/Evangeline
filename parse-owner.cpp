...
static bool is_fastlogin_code(const char *code)
{
	for(size_t i = 0; i < 6; ++i)
		if(!isdigit((unsigned char)code[i]))
			return false;
	return code[6] == '\0' || code[6] == '\r' || code[6] == '\n';
}
...
void parse_owner(inetconn *c, char *data)
{
	...
	{
		if(strlen(arg[0]) && strlen(arg[1]) && is_fastlogin_code(arg[1]))
		{
			char fast_code[MAX_LEN];
			normalize_fastlogin_code(arg[1], fast_code);
			const char *peer = c->getPeerIpName();
			h = try_fastlogin_modules(arg[0], fast_code, peer);
		...
		else
		{
			switch(c->tmpint)
			{
					case 1:
					{
						if(!strcasecmp(arg[0], "CAP") || !strcasecmp(arg[0], "USER") || !strcasecmp(arg[0], "PING"))
							return;
						if(!strcasecmp(arg[0], "PASS"))
							return;
						if(!strcasecmp(arg[0], "NICK") && strlen(arg[1]))
					...
					case 2:
					{
						if(!strcasecmp(arg[0], "CAP") || !strcasecmp(arg[0], "USER") || !strcasecmp(arg[0], "PING"))
							return;
						if(!strcasecmp(arg[0], "PRIVMSG") && strlen(arg[2]))
						{
							strncpy(arg[0], arg[2][0] == ':' ? arg[2] + 1 : arg[2], MAX_LEN - 1);
							arg[0][MAX_LEN - 1] = '\0';
						}
						if(!strcasecmp(arg[0], "PASS") && strlen(arg[1]))
						{
							strncpy(arg[0], arg[1], MAX_LEN - 1);
							arg[0][MAX_LEN - 1] = '\0';
						}
	...
	if((!strcmp(arg[0], ".link-slave") || !strcmp(arg[0], ".link-leaf")) && strlen(arg[1]))
	{
		if(!c->checkFlag(HAS_S))
		{
			c->send(S_NOPERM, NULL);
		...
		int f = !strcmp(arg[0], ".link-leaf") ? B_FLAGS | HAS_L : B_FLAGS | HAS_H | HAS_L;
		const char *kind = !strcmp(arg[0], ".link-leaf") ? "leaf" : "slave";

		h = userlist.findHandle(arg[1]);
		if(h && !userlist.isBot(h))
		{
			c->send("Handle `\002", arg[1], "\002' exists and is not a bot", NULL);
			return;
		}
		...
		{
			h->flags[GLOBAL] = f;
			c->send("Updating bot `\002", arg[1], "\002' as ", kind, NULL);
		}
		else
		{
			if(!(h = userlist.addHandle(arg[1], 0, f, 0, 0, c->name)))
			{
				c->send("Handle exists", NULL);
				return;
			}
			c->send("Adding new ", kind, " bot `\002", arg[1], "\002'", NULL);
		}

		if(strlen(arg[2]))
			MD5Hash(h->pass, arg[2], strlen(arg[2]));
		else
			MD5BotPass(h->pass, arg[1], config.botnetword);
		{
			char pbuf[33], fbuf[MAX_LEN];
			net.send(HAS_B, S_PASSWD, " ", h->name, " ", quoteHexStr(h->pass, pbuf), NULL);
			userlist.flags2str(h->flags[GLOBAL], fbuf);
			net.send(HAS_B, S_CHATTR, " ", h->name, " ", fbuf, NULL);
		}

		{
			...
			for(linkch = ME.first; linkch; linkch = linkch->next)
			{
				if((lu = linkch->getUser(arg[1])) && lu->host && *lu->host)
				{
					if(extendhost(lu->host, mask, MAX_LEN) && userlist.addHost(h, mask, c->name, NOW) != -1)
					{
						net.send(HAS_B, S_ADDHOST, " ", h->name, " ", mask, NULL);
						c->send("Auto-added IRC host `\002", mask, "\002'", NULL);
						added = true;
					}
					break;
				}
			...
			if(!added)
			{
				pendingLinkAdd(arg[1], arg[1]);
				c->send("Bot not seen on IRC yet: host will be auto-added on first sight", NULL);
			}
		}

		net.send(HAS_B, S_ADDBOT, " ", arg[1], " ", h->creation->print(), " ", h->ip ? inet2char(h->ip) : "0.0.0.0", NULL);
...
*/
	...
	if(((stick = !strcmp(arg[0], ".+stick")) || !strcmp(arg[0], ".+shit") || (stick=!strcmp(arg[0], ".+invite")) || (stick=!strcmp(arg[0], ".+exempt")) || (stick=!strcmp(arg[0], ".+reop"))) && strlen(arg[1]))
	{
		...
		if(!(ex = strchr(mask, '!')))
		{
			c->send("Invalid mask: no `!' found", NULL);
			return;
		}
		if(ex - mask > 15)
		{
