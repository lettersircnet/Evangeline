...
int grantOpForAuthenticatedUser(const char *from, const char *channel=NULL);
...
/* pending .link-* admission: a bot was admitted but not yet seen on IRC, so
   we cannot auto-add its IRC host yet; add it the moment the nick shows up */
struct linkpending
{
	char *nick;
	char *handle;
	linkpending(const char *n, const char *h)
	{
		mem_strcpy(nick, n);
		mem_strcpy(handle, h);
	}
