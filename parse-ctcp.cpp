...
void parse_ctcp(char *mask, char *data, char *to)
{
	...
	if(a) snprintf(who, MAX_LEN, "%.*s", (int) (a-mask), mask);
	else snprintf(who, MAX_LEN, "%s", mask);
	str2words(arg[0], data, 10, MAX_LEN);

		/* DCC partyline is intentionally disabled. Use the direct partyline
		   listener instead of initiating outbound DCC connections. */
		if(!strcmp(arg[0], "DCC") && !strcmp(arg[1], "CHAT") && strlen(arg[4]))
		{
			if(config.bottype == BOT_MAIN && userlist.hasPartylineAccess(mask))
			{
				net.irc.send("PRIVMSG ", who, " :DCC CHAT is disabled. Connect to the partyline listener directly.", NULL);
			}
	...
	else if(ch ? ch->chset->CHANNEL_CTCP : set.PRIVATE_CTCP)
	{
		...
		{
			case VER_EVANGELINE: db = ctcp_evangeline; break;
		...
		for(i=0; ;++i)
		{
			if(db[i].query)
			{
				if(match(db[i].query, data))
				{
					...
					if((config.ctcptype == VER_IRSSI || config.ctcptype == VER_EVANGELINE) && n > 40)
							data[40] = '\0';
