...
static bool configLooksEncrypted(const char *file)
{
	...
	return n >= sizeof(h.id) && (!memcmp(h.id, oldMagic, sizeof(h.id)) || !memcmp(h.id, newMagic, sizeof(h.id)));
}
...
void parse_cmdline(int argc, char *argv[])
{
	...
	{
#ifdef HAVE_DEBUG
		printf("Syntax: %s [-v] [-a] [-d] [-p] [-u] [-n] [-c|-T decrypted config] [config]\n", argv[0]);
#else
			printf("Syntax: %s [-v] [-a] [-p] [-n] [-c|-T decrypted config] [config]\n", argv[0]);
#endif
		exit(1);
	}
	...
	for(i=1; i<argc; ++i)
	{
		...
		else if(!strcmp(argv[i], "--crypto-tests"))
		{
			...
			{
				if(!creation && !decrypted && !configLooksEncrypted(argv[i]))
				{
					autocrypt = 1;
					decrypted = true;
				}
				config.load(argv[i], decrypted);
			}
			else
		{
...
/* wall-clock milliseconds; used for RTT/lag metering of PING/PONG probes
   (resolution-independent of the 1s NOW clock) */
...
void sendLogo(inetconn *c)
{
	...
	c->send("", NULL);
	c->send("\002      _/_/_/ _/  _/  _/_/   _/   _/ _/_/_/ _/_/_/ _/     _/ _/   _/ _/_/_/  \002", NULL);
	c->send("\002     _/     _/  _/ _/   _/ _//  _/ _/     _/     _/        _//  _/ _/       \002", NULL);
	c->send("\002    _/_/   _/  _/ _/_//_/ _//_/_/ _/_/_/ _/_/   _/     _/ _//_/_/ _/_/      \002", NULL);
	c->send("\002   _/     _/  _/ _/   _/ _/  /_/ _/  _/ _/     _/     _/ _/  /_/ _/        \002", NULL);
	c->send("\002  _/_/_/  _/_/  _/   _/ _/   _/ _/_/_/ _/_/_/ _/_/_/ _/ _/   _/ _/_/_/      \002", NULL);
	c->send("\002                    'MESS WITH THE BEST, DIE LIKE THE REST'                  \002", NULL);
	c->send("", NULL);
	c->send("   Evangeline \002", S_DISPLAY_VERSION, "\002", NULL);
	c->send("   Don't abuse it, and it will be opened for all your friends, too", NULL);
	c->send("   ", S_COPYRIGHT, NULL);
	c->send("   ", S_COPYRIGHT2, NULL);
	c->send("   ", S_COPYRIGHT3, NULL);
	c->send("", NULL);
	...
	if(c->checkFlag(HAS_N))
	{
		...
		c->send("Session: \002", c->name, "\002 on \002", (const char *) config.handle, "\002", NULL);
		c->send("Runtime: \002", name.sysname, " ", name.release, " ", name.machine, "\002", NULL);
		c->send("Clock: \002", timestr, "\002", NULL);
		c->send("Network: owners \002", itoa(net.owners()), "\002 | bots \002", itoa(net.bots() + 1), "\002", NULL);
		c->send("Commands: .owners .upbots .help .quit", NULL);
		if(i)
	...
	{
		net.send(HAS_N, "[!] ", c->name, " tried to enter the partyline without privileges", NULL);
		c->close("partyline privileges required");
	}
}
...
void propaganda()
{
	printf("\n");
#ifdef HAVE_IPV6
	printf("Evangeline %s IPv6 (rev: %s, build: %s %s)", S_DISPLAY_VERSION, SVN_REVISION, __DATE__, __TIME__);
#else
	printf("Evangeline %s (rev: %s build: %s %s)", S_DISPLAY_VERSION, SVN_REVISION, __DATE__, __TIME__);
#endif
	printf("\n");
	printf(S_COPYRIGHT);
	printf("\n");
...
ptrlist<linkpending> pendingLinks;

void pendingLinkAdd(const char *nick, const char *handle)
{
	if(nick && *nick)
		pendingLinks.add(new linkpending(nick, handle ? handle : nick));
}

int pendingLinkTry(const char *nick, const char *host)
{
	ptrlist<linkpending>::iterator p = pendingLinks.begin(), ex;
	char buf[MAX_LEN];
	HANDLE *h;
	...
	while(p)
	{
		...
		if(!strcasecmp(p->nick, nick))
		{
			...
			h = userlist.findHandle(p->handle);
			if(h && host && *host && extendhost(host, buf, MAX_LEN) && userlist.addHost(h, buf, "link", NOW) != -1)
			{
				net.send(HAS_B, S_ADDHOST, " ", h->name, " ", buf, NULL);
				++userlist.SN;
				net.send(HAS_N, "[*] ", h->name, ": IRC host \002", buf, "\002 added", NULL);
			}
			return 1;
		}
		p = ex;
...
int acceptConnection(int fd, bool ssl)
{
	...
	fromsize = sizeof(from6);
	if((n = accept(fd, (sockaddr *) &from6, &fromsize)) < 1)
		return 0;
	...
	{
		memcpy(&from, &from6, sizeof(from));
		fromsize = sizeof(from);
	}
	{
		ign::entry *e = peer6 ? ignore.hit(0) : ignore.hit(ntohl(from.sin_addr.s_addr));
		...
		if(e->nextConn >= NOW || ignore.nextConn >= NOW)
		{
			...
			{
	        	if(peer6 || userlist.isBot(from.sin_addr.s_addr) || set.TELNET_OWNERS)
				{
					fcntl(n, F_SETFL, O_NONBLOCK);
					setsockopt(n, SOL_SOCKET, SO_KEEPALIVE, &one, sizeof(one));
					setsockopt(n, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one));
				c = net.addConn(n);
				c->tmpint = 1;
...
{
#ifdef HAVE_IPV6
	if(getIpVersion(fd) == AF_INET6)
	{
		struct sockaddr_in6 peer6;
		#ifdef _NO_LAME_ERRNO
		int ret, e = errno;
		#endif
		socklen_t peer6size = sizeof(struct sockaddr_in6);
		ret = fun(fd, (sockaddr *) &peer6, &peer6size);
		#ifdef _NO_LAME_ERRNO
		errno = e;
		#endif
		if(ret == -1) return 0;
		else return ntohs(peer6.sin6_port);
	}
#endif
	struct sockaddr_in peer;
	#ifdef _NO_LAME_ERRNO
...
#ifdef HAVE_IPV6
int startListening6(const char *ip, int port)
{
	struct sockaddr_in6 sin6;
	int s;
	const int one = 1;
...
void precache_expand()
{
	...
	{
		case VER_EVANGELINE: expandinfo.version = S_VERSION; break;
...
void addToCron(int i, char *argv[], int argc)
{
	...
	if(!fclose(p))
	{
		printf("[+] Added %d evangeline bot%s to cron\n", n, (n == 1 ? "" : "s"));
		return;
	}
	else
	{
...
{
	strcpy(__versionStr, S_BOTNAME "-" S_VERSION "-");
